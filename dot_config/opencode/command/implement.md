...
Example: .opencode/thoughts/1234567890_dark_mode/plan.md
...
3. **Verify git state** (if in a git repository):
   - Use the `git-context` tool to check current state
   - Verify working directory is clean (or document what's uncommitted)
   - Check if branch matches the one from plan/research
   - Note any new commits since planning
   - Warn if there are uncommitted changes that might conflict
   ...
   [List from git-context]
   
   These might conflict with the implementation. Options:
   1. Commit these changes first
   2. Stash them temporarily
   3. Proceed carefully (I'll track both sets of changes)
...
Using git-context to verify clean state...

**Git Status**:
- Branch: [current branch]
- Modified files: [files changed during implementation]
- New files: [files created]
- Deleted files: [files removed]
- All changes staged: [yes/no]
...
- Research: .opencode/thoughts/[epoch]_[name]/research.md
- Plan: .opencode/thoughts/[epoch]_[name]/plan.md
