...
Example: .opencode/thoughts/1234567890_dark_mode/research.md
...
1. **If research.md path is provided**:
   ...
   - Extract the research directory path (e.g., `.opencode/thoughts/1234567890_dark_mode`)
   - Note the short name and epoch from the directory
   - Understand all findings, patterns, and constraints
   - Extract git context from research (branch, recent commits, pending changes)

2. **Capture current git context** (if in a git repository):
   - Use the `git-context` tool to get the latest state
   - Check if state has changed since research was conducted
   - Note any new commits or changes since research
   - Identify if there are uncommitted changes to account for
   - Include this information in the plan

3. **If no research provided**:
...
Write plan.md to the same directory as research.md (extract the directory path from the research.md file path):
...
**Branch**: [current branch from git-context]
**Recent Commits**: 
- [Commit 1 hash] - [message] ([time ago])
- [Commit 2 hash] - [message] ([time ago])
- [Commit 3 hash] - [message] ([time ago])

**Uncommitted Changes**: [List files modified but not committed, or "none"]
**Working Directory**: [clean | changes pending]

This context informs the plan:
- Shows what work is already in progress that we should coordinate with
- Identifies files that are actively being modified (potential conflicts)
- Provides context about recent development focus and priorities
- Helps determine if we should account for pending changes in our approach
...
- [ ] Tests pass: `bun test` (or specific test command)
...
|------|-------|----------------|
| `path/to/file1.*` | 1 | Modify existing function |
| `path/to/file2.*` | 1 | Add new component |
| `path/to/test_file.*` | 2 | Add tests |
| `path/to/file3.*` | 3 | Update integration |
...
- Maintain the directory structure: `.opencode/thoughts/{epoch}_{name}/`
...
- Specific commands for verification (use `bun` commands)
