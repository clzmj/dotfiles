...
### Main Implementation: `path/to/file.*:line`
...
**Depends on**:
- `ComponentA` (path/to/component_a.*:10) - [How it's used]
- `ServiceB` (path/to/service_b.*:25) - [How it's used]

**Used by**:
- `ComponentC` (path/to/component_c.*:30) - [How it uses this component]

### State Management

[How state is managed - global state, local state, state patterns, etc.]
- State structure: [What the state looks like]
- Updates: [How state is modified]
- Location: `path/to/state.*:line`
